...
class bin_id3(object):
    ...
    def entropy(examples, attrib, avt):
        ...
        """
        proportions = (bin_id3.proportion(examples, attrib, val)
    ...
    def gain(examples, target_attrib, attrib, avt):
        ...
        def reduction(val):
            split = bin_id3.find_examples_given_attrib_val(
                examples, attrib, val)
            return (
                len(split) / len(examples) *
                bin_id3.entropy(split, target_attrib, avt)
            )
    ...
    def fit(examples, target_attrib, attribs, avt, dbg):
        ...
        if len(attribs) == 1:
            return id3_node(
                bin_id3.find_most_common_attrib_val(
                    examples, target_attrib, avt)[0]
            )
